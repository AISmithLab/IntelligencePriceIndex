...
| U7 | "What does GMV mean" / "do you mean transactions or revenue — I am confused" | 2026-08-20 | PASS | `drafts/plain-summary.md` §"Six words that get mixed up" — a six-row table separating **buyers** (people), **orders** (purchases, *published by nobody*), **GMV** (dollars transacted), **revenue** (Fiverr's cut, ~1/3 of GMV), **review accrual** (our sales proxy) and **listed vs realised price**, plus the two consequences: GMV = buyers x spend per buyer is arithmetic, and every transactions-fell claim in the document is derived rather than reported. §2.2 also spells GMV out on first use and now states the implied order count (−18%) with its upper-bound caveat. **Raised by the reader twice in succession, on a document whose stated purpose was to be readable without jargon — the failure mode is assuming finance vocabulary is not jargon** |
