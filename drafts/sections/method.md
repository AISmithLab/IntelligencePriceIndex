...
**How this section is organised.** §3.1 states what the data are and how the source and the sampling frame were chosen. §3.2 then narrates the collection **in the order it actually happened** — each step forced by what the step before it revealed — before giving the finished pipeline as a table and condensing the changes into an auditable record. §3.3–3.5 build the index from the resulting panel; §3.6–3.7 report how well it is identified; §3.8–3.10 measure the rival explanations and list what earlier versions of this work got wrong.

*Terminology, fixed here for the whole section.* Through §3.1–3.2, **index** unqualified means the Wayback Machine's capture index — its catalogue of archived pages; from §3.3 onward it means the price index we estimate. **Pilot** unqualified means the 500-seller pilot crawl of §3.2, not the 20-page feasibility probe of §3.1.
...
#### What the census then revealed, and why it constrains everything downstream

The census answered a question the feasibility probe could not, and answered it badly. The third property above is weaker than it appears: *the archive is an opportunistic crawl, not a sampling frame.* Pages are captured when the Internet Archive happens to capture them, at rates varying by page popularity and by year, and nothing about the resulting density is designed. Twenty pages cannot detect this; a census of 60 million capture records makes it unmissable.
...
That is the first of several points at which the collection was redesigned around something the data had just revealed. Section 3.2 gives the rest in the order they occurred.
...
#### The collection as it happened

The pipeline given below is the collection as it now stands. It is not how it was built, and reporting only the final state would misrepresent what work of this kind costs. **Each step below was forced by something the step before it revealed**, and we give them in order so that a reader can check that no change was made in order to obtain a result.

**1 — Harvest the catalogue.** With the two-phase decision of §3.1 made, the first operation was the census: 60 million capture-index records, retrieved per first-letter prefix. It produced both the archive's true size and the 2018Q3 floor.

**2 — A sampling rule written, then abandoned (Mar 2026).** The first design stratified sellers by how often the archive had captured them. That rule selects on the Internet Archive's crawl intensity — how interesting a page was to a crawler — rather than on whether a seller is useful for a price panel. We replaced it with a qualification on longitudinal depth, then a uniform random draw. Earlier drafts described the result as a *stratified* sample; it is not, and §3.10 corrects that.

**3 — The pilot crawl.** 48,643 sellers qualified, 500 were drawn, and their snapshots were downloaded, priced, and collapsed into a panel of 1,066 gigs. **Every historical figure in this paper comes from this one crawl.**

**4 — What the pilot could not do, and the second crawl (Jun 2026).** Two failures surfaced, both structural rather than reparable. Sellers selected for *long histories* are precisely those whose captures thin at the trailing edge, so the pilot goes sparse after 2024Q4 and cannot measure the recent period at all. And no crawl of this archive reaches before 2018Q3. The first was answered by a second crawl — anchored at 2024Q3, requiring a capture in the trailing window — giving **15,309 snapshots over 3,589 gigs** at far higher density than the pilot achieves anywhere. This is why the paper has a splice (§3.4), and nearly every asymmetry in its results, including which categories are identified at all (§3.7), traces to one crawl being selected for depth and the other for density. The second failure was not answered, because it cannot be.

**5 — Two operational lessons (Jul–Aug 2026).** The recent crawl had run at 20 concurrent requests and 20/second, logging **12,336 transient failures against 15,150 successes** — a 45% per-attempt failure rate, absorbed only because failures are un-checkpointed. A later pilot at 10 and 10/second logged **zero** at the same sustained 5.71 pages/second. Exactly **3** of both crawls' 38,000-odd logged responses were 403s, so we have no evidence of having been blocked; the correct reading is that we had been running well past diminishing returns and did not know it until it was measured. Separately, storage moved to gzip, measured at 5.0× on this corpus — the difference between 93 GB and 17.6 GB.

**6 — A parsing defect, and a retraction (Aug 2026).** Several of Fiverr's own section pages satisfy the gig URL rule, and the extractor had been recording a budget-filter default on them as though it were a price (Stage 5b below). Removing them retracted a published finding, which §3.10 and §4.6 treat in full. What belongs here is how it was caught: the defect survived every check we had until the extraction-method **share** table made it visible. That is why Stage 5 reports method shares rather than only its 100% success rate — a success rate would have shown nothing wrong.

**7 — The binding constraint located (Aug 2026).** Re-read against the shipped panel, the census showed that the limiting factor had never been the archive but our own selection rule, and that the quantity we had been counting — panel gigs — is not the one that governs precision. Two enlarged collections followed; they are described at the end of this section and **contribute no number to this paper**.
...
| Phase | Stage | Operation | In → out (historical crawl) |
|---|---|---|---|
| 1 | 1 | Retrieve the Wayback capture index per first-letter prefix, 3 concurrent queries with backoff (`code/01`) | → **60M** capture-index entries |
| 2 | 2 | URL shape (2 path segments, status 200, ~60 reserved first segments); dedup to one record per (URL, day), dropping identical content digests; classify slugs against `data/task-taxonomy.md`; keep sellers holding a gig with ≥5 monthly snapshots over ≥2 years (`code/02`–`05`) | 60M → **22.7M** unique (URL, month); **48,643** qualifying sellers |
| 2 | 3 | Uniform random draw, fixed seed: 48,643 → 5,000 → **500 sellers**; take all their gig snapshots, thinned to one per gig-month (`code/06c`, `07`) | → **26,603** snapshots over 14,938 gigs |
| 3 | 4 | Download raw captures at 12 req/s with backoff, stored gzipped (`code/08`) | 26,603 → **22,632 (85.1%)**; recent crawl 15,309 → 15,150 (99.0%) |
| 4 | 5 | Four-method extraction cascade (`code/09`) | 22,632 → **22,632 (100%)** |
| 4 | 5b | Drop reserved-segment pages that are not gigs (`code/gigfilter.py`) | 37,782 → **33,936** across both crawls (−10.2%) |
| 5 | 6 | Collapse to unique (seller, slug) with ≥1 valid price | → **1,908** unique gigs |
| 5 | 7 | TF-IDF titles + agglomerative clustering into 150 service items (`code/10`) | descriptive use only |
| 5 | 8 | Gig-quarter median, price guard 0 < *p* ≤ \$10,000, keep gigs in ≥2 quarters | → **1,066 historical**, 2,908 recent panel gigs |

Stages 1–3 consume the capture index alone; only Stage 4 requests a page. Both crawls run the same Stages 4–8 and differ only in the Stage 3 selection rule, which is why the in→out column reports the historical crawl and gives the recent crawl's counterpart inline where the two differ.
...
The narrative above and §3.1's scoping decision are condensed here as one auditable record, so that a reader checking what was changed and why does not have to reassemble it from the prose. Changes 2, 3, 5, 6 and 7 are the pipeline revisions narrated above; 1, 4 and 8 are design responses of the same kind, made at other points in the project.
...
Rows 5 and 7 carry a caution the table cannot. **The rate lesson cost nothing, but that was luck** — a 45% failure rate was survivable only because failures are un-checkpointed, and a pipeline that checkpointed them would have lost a third of the crawl. **The exclusion in row 7 is the only change that moved a published number** rather than the machinery around it, and it was found by an instrument we happened to have rather than by one we had designed for it.
