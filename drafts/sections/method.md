...
The data were collected in six phases, in this order. The ordering is itself a design decision and not an incidental one: **phases 1 and 2 complete before a single page is downloaded**, so the sample is drawn against a census of what the archive holds rather than against whatever a crawler reached first.

| Phase | What was done | Detailed in |
|---|---|---|
| 0 | **Choose the source.** 20-page feasibility probe against three pass/fail criteria fixed in advance; Upwork and Freelancer.com probed as fallbacks and rejected | §3.1 |
| 1 | **Census the archive.** Retrieve the Wayback *capture index* — the catalogue of what is archived, carrying no page content | §3.1; §3.2 Stage 1 |
| 2 | **Build the manifest offline.** Filter to gig-shaped URLs, deduplicate, classify, qualify sellers on longitudinal depth, draw the sample | §3.2 Stages 2–3 |
| 3 | **Download.** Fetch only the pages the manifest names | §3.2 Stage 4 |
| 4 | **Read the prices.** Four-method extraction cascade, then exclude the page families that are not gigs | §3.2 Stages 5–5b |
| 5 | **Build the panel.** Collapse to unique gigs, take gig-quarter medians, keep gigs observed in ≥2 quarters | §3.2 Stages 6–8 |

Two things cut across that order and are easy to mistake for further steps. **The pipeline was run twice, not once** — a historical crawl and a recent crawl, differing only in the phase-2 selection rule and estimated separately thereafter ("Two crawls, not one" below). And **the pipeline was revised five times while it ran**; those revisions are chronological rather than sequential, and are given under "How the collection changed" in §3.2.

*Terminology, fixed here for the whole section.* Through §3.1–3.2, **index** unqualified means the Wayback capture index of phase 1; from §3.3 onward it means the price index we estimate. **Pilot** unqualified means the 500-seller pilot crawl of phase 2, not the phase-0 feasibility probe (see §3.2).
