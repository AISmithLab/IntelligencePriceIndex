...
**Reputation accumulation ("the treadmill").** A gig's price rises partly because it accumulates reviews, not because the service became dearer. We estimate this two ways, on two panels, and they agree.

The primary estimate is a two-way fixed-effects model on the **balanced panel** — every gig observed both before and after 2022Q4, giving **169,337 gig-quarter observations across 15,676 gigs over 2019Q4–2024Q4**, with prices deflated to 2020Q1 dollars as in §3.5. Within a gig-quarter, price and rating take the median and the cumulative review count takes the quarter-end maximum. We fit

$$\ln p^{real}_{i,t} = \alpha_i + \delta_t + \beta_1 \ln(1 + R_{i,t}) + \beta_2 \,\text{rating}_{i,t} + \varepsilon_{i,t}$$

with gig and quarter fixed effects and gig-clustered standard errors, giving $\beta_1 = +0.1021$ (se 0.0039, $t = 25.95$) — **+7.33% per doubling of reviews** — with within-gig $R^2 = 0.135$ and a residual SD of 0.348 log points. The gig fixed effect $\alpha_i$ is itself interpretable as **task value**: the log real price a particular piece of work commands net of inflation, the common quarter path, and reputation. It spans 1.249 log points of standard deviation across the 15,676 gigs — far more variation than anything time-varying in the model, which is why it belongs in a fixed effect rather than a proxy.

The pilot estimate, on first differences, is $\beta = +0.1068$ ($t = 5.32$) over 9,762 transitions — **+7.7% per doubling** in nominal terms. That the two agree to within half a percentage point across a fiftyfold change in sample size and a change from nominal to real terms is the strongest internal validation the paper carries.

Two qualifications belong with the coefficient. **It is not a constant rate.** Strict log-linearity is rejected; the implied slope tapers from roughly +8.1% per doubling at 10 reviews to +4.7% at 5,000, so the pooled figure is an average over the mass (the median gig-quarter carries 111 reviews) rather than a rate. **And linear tenure is not separably identified** from gig and quarter fixed effects, since it decomposes into a gig component and a time component; $\beta_1$ is identified off curvature in review accumulation, and removing each gig's own linear trend leaves 45.4% of the within-gig variation and raises the coefficient to +0.150 ($t = 30$). Neither qualification changes the sign or the order of magnitude, and the index adjustment below uses the pilot $\beta$ for continuity with the published series. We then publish a **band**: raw, and reputation-adjusted by rescaling each cell as $p \cdot \exp(-\beta \ln(1+R))$ and re-estimating. The composite runs **+79.0% raw and +39.7% adjusted**. Three properties travel with it. The adjusted series is a **lower bound, not a correction** — reviews are cumulative sales, so $\beta$ absorbs demand as well as reputation, and if AI suppressed demand the adjustment consumes part of the effect the index is trying to show; raw remains the headline. $\beta$ is **pooled**, forced rather than preferred: estimated per category, audio and translation return the wrong sign. And the **floor is soft** — across $\beta$'s own 95% interval the adjusted bound moves between roughly +50% and +28%, so it must not be quoted as a single number.
...
**The pre-registered test for an AI effect.** The specification registered in advance adds to the model above an interaction between a category's AI exposure and a post-launch indicator:

$$\ln p^{real}_{i,t} = \alpha_i + \delta_t + \beta_1 \ln(1 + R_{i,t}) + \beta_2 \,\text{rating}_{i,t} + \theta \,(E_{c(i)} \times \text{Post}_t) + \varepsilon_{i,t}$$

where $E_c$ is Eloundou et al.'s human-annotated occupational exposure mapped to our seven categories, constant within a category, and $\text{Post}_t = \mathbb{1}[t \geq 2022\text{Q}4]$. Exposure runs from 0.25 (audio) to 0.84 (translation). Because $E_c$ is time-invariant and $\text{Post}_t$ is category-invariant, both main effects are absorbed by the fixed effects and only the interaction is identified. §4.8 reports $\hat\theta$ and the six diagnostic gates specified with it — parallel trends, a linear-trend horse race, a placebo break inside the pre-period, first differences, clustered versus unclustered inference, and a minimum-detectable-effect calculation. **The gates were fixed before estimation and we report all six outcomes including the three failures**, because a null whose identifying assumptions fail is a different object from a null whose assumptions hold.

We also report a **secondary, explicitly exploratory** specification in which exposure is measured from the market rather than assigned — the share of a category's listings that brand themselves as AI services, varying by category *and* quarter. It is not pre-registered, it ranks the categories nearly opposite to the registered measure, and it therefore cannot be promoted to primary after the fact. We report it because it produces a large, tightly-estimated, and wrong coefficient, and because the diagnostic that exposes it generalises: **any regressor varying at the category-by-quarter level absorbs any category-specific time path**, so it must be raced against category-specific linear trends before it is believed. §4.8 gives both numbers.
...
**One correction changed a published finding.** We had reported a real −21% fall in cognitive-labor prices in 2025 from a composite peak of 312. It was an artifact of two defects now removed — the naive chained series (§3.4) and the non-gig section pages (§3.2). **It is retracted**, and §4.7 gives the before-and-after. Separately, the `rating` column carries a scale error in **217 historical rows** from Fiverr's pre-2019 10-point scale; it touches no result but is not yet fixed upstream.
